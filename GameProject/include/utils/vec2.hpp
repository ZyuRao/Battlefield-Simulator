...
struct Coord {
    ...
    int mhtDistanceTo(const Coord& other) const {
        return std::abs(x - other.x) + std::abs(y - other.y);
    }

    static int mhtDistance(const Coord& a, const Coord& b) {
...
// ================== Design Notes ==================
// 模块功能：
// Coord 是整个项目中的基础二维坐标结构，描述地图格子、单位位置、基地位置等。
// 该类型非常轻量，采用 struct + inline 成员函数，性能高、易复用。

// 采用的模式：
// 无特定设计模式，但遵循轻量值类型（Value Type）设计原则。
// 运算符重载使 Coord 在地图、移动、AI 路径等模块中更易使用。
